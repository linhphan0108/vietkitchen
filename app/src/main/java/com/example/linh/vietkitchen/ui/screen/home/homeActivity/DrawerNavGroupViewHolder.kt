...
import kotlinx.android.synthetic.main.item_group_drawer_nav.view.*
...
class DrawerNavGroupViewHolder(itemView: View, private val listener: OnGroupItemClickListener? = null)
    ...
    fun bindView(item: DrawerNavGroupItem, payloads: MutableList<Any>) {
        if (payloads.isNotEmpty()){
            ...
            itemView.isSelected = item.isSelected
            itemView.txtGroupTitle.text = context.getString(R.string.drawer_nav_item_title, item.headerTile, item.numberItems)
            ...
            if (item.itemsList.isNullOrEmpty()){
                itemView.txtGroupTitle.setCompoundDrawablesWithIntrinsicBounds(null, null, null, null)
                ...
                itemView.txtGroupTitle.setCompoundDrawablesWithIntrinsicBounds(startDrawable, null, null, null)
            }

            //since some empty group won't have compound drawable
            //so that reset the padding
            val topPadding = itemView.txtGroupTitle.paddingTop
            val endPadding = itemView.txtGroupTitle.paddingEnd
            val bottomPadding = itemView.txtGroupTitle.paddingBottom
            ...
            itemView.txtGroupTitle.setPadding(startPadding, topPadding, endPadding, bottomPadding)
        }
    }
}
